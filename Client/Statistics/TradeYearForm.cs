...
{
    public partial class TradeYearForm : Form
    {
        #region 窗口初始化
        /// <summary>
        /// 窗口初始化
        /// </summary>
        public TradeYearForm()
        {
            InitializeComponent();
        ...
        {
            this.dataGridView.DataSource = VehicleDao.CaculateTradeYearReport(this.startDate.Value.Date.AddMonths(1-this.startDate.Value.Month).AddDays(1 - this.startDate.Value.Date.Day).AddMilliseconds(-1),
                this.endDate.Value.Date.AddYears(1).AddMonths(1 - this.startDate.Value.Month).AddDays(1 - this.endDate.Value.Date.Day).AddMilliseconds(-1)).Tables[0];
        }
        #endregion
