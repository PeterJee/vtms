...
namespace NHibernate.Impl
{
	...
	public sealed class SessionImpl : AbstractSessionImpl, IEventSource, ISerializable, IDeserializationCallback
	{
        ...
        public override DataSet QueryDataSet(NativeSQLQuerySpecification spec, QueryParameters queryParameters)
        {
            using (new SessionIdLoggingContext(SessionId))
            {
                ...
                SQLCustomQuery query = new SQLCustomQuery(
                    ...
                    spec.QuerySpaces,
                    Factory);
                DataSetCustomQuery(query, queryParameters, ref results);
                return results;
            }
        }

        private void DataSetCustomQuery(SQLCustomQuery customQuery, QueryParameters queryParameters, ref DataSet results)
        {
            using (new SessionIdLoggingContext(SessionId))
            {
                CheckAndUpdateSessionStatus();
                ...
                {
                    //ArrayHelper.AddAll(results, loader.List(this, queryParameters));
                    results = loader.QueryDataSet(this, queryParameters);
                    success = true;
                }
                finally
                {
