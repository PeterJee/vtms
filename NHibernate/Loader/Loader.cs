	...
	public abstract class Loader
	{
        ...
        protected DataSet QueryDataSet(Impl.SessionImpl sessionImpl, QueryParameters queryParameters, ISet<string> querySpaces)
        {
            ...
            try
            {
                try
                {
                    result = DoDataSetQuery(sessionImpl, queryParameters);
                }
                finally
                {
                    persistenceContext.AfterLoad();
        ...
        private DataSet DoDataSetQuery(ISessionImplementor session, QueryParameters queryParameters)
        {
            DataSet ds = new DataSet();
            using (IDbCommand st = PrepareQueryCommand(queryParameters, false, session))
            {
                IDbConnection connection = session.Connection;
                st.Connection = connection;
                IDbDataAdapter adapter = session.Batcher.CreateAdapter(st);
