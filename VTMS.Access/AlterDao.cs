using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using NHibernate;
using NHibernate.Cfg;
using VTMS.Model.Entities;

namespace VTMS.Access
{
    public class AlterDao
    {
        public static string GetLatestSerial()
        {
            ISession session = SessionFactory.OpenSession();
            string serial = session.CreateSQLQuery(
                        @"SELECT IF(NOT EXISTS(SELECT * FROM vtms.alters WHERE serial = CONCAT(DATE_FORMAT(CURRENT_DATE,'%Y%m%d'),'001')),
                            CONCAT(DATE_FORMAT(CURRENT_DATE,'%Y%m%d'),'001'),
                            (SELECT MAX(serial)+1 FROM vtms.alters)) as serial"
                        ).List()[0].ToString();
            return serial;
        }
        public static object Add(Alter entity)
        {
            entity.Serial = GetLatestSerial();
            ISession session = SessionFactory.OpenSession();
            var id = session.Save(entity);
            session.Flush();
            return id;
        }

        public static void Update(Alter entity)
        {
            ISession session = SessionFactory.OpenSession();
            session.Update(entity);
            session.Flush();
        }

        public static void Delete(Alter entity)
        {
            ISession session = SessionFactory.OpenSession();
            session.Delete(entity);
            session.Flush();
        }

        public static Alter GetBySerial(string id)
        {
            ISession session = SessionFactory.OpenSession();
            return session.Get<Alter>(id);
        }
    }
}
